...
class DesignTwo extends PdfService implements PdfContract
{
    /** @var \TCPDF */
     ...
     * {@inheritdoc}
     */
    public function addAppendixPage(
        Company $company,
        ReportDraft $reportDraft
    ): PdfContract {
...
public function addTeamPage(
        ...
        $incrementXunit = $startPointX + $spaceW;
        ...
        foreach(collect($reportDraft->teams) as $team){
            ...
            // self::$pdf->lastPage();
            // $incrementYunit = self::$pdf->pixelsToUnits($incrementY);
            // if(self::$pdf->getPageHeight() - $incrementYunit <= 50)
            // {
            //     self::$pdf->lastPage();
            // }
            // 
            // dd($incrementY);
            // dd(collect($reportDraft->teams)->last());

            // self::$pdf->getLastH();
            // self::$pdf->Cell(0,0, self::$pdf->getLastH(),0,1);
     ...
     * {@inheritdoc}
     */
    public static function makeCustom(
        User $user,
        ReportDraft $reportDraft,
        int $total
    ): PdfContract {
    ...
    public function addCompanyPage(
        ...
        foreach ($descriptions as $index => $description) {
            ...
            self::$pdf->SetTextColor(
                self::$pdf->rgbaColor('white')['r'],
                self::$pdf->rgbaColor('white')['g'],
                self::$pdf->rgbaColor('white')['b']
            );
            self::$pdf->SetAlpha(self::$pdf->rgbaColor('white')['a']);
            ...
            self::$pdf->LinearGradientTransparent(
                $this->pageStartX,
                $this->pageStartY + $galleryHeight - 35,
                self::$pdf->pixelsToUnits(675),
                33,
                [0],
                [0],
                [0, 0, 0, 1]
            ...
            self::$pdf->writeHTMLCell(
                ...
                $this->pageStartX + 5,
                $galleryHeight - 15,
                view('cms.report.partials.pdf.company-header-2', compact('company', 'name', 'companyAbout'))->render(),
                0,
                0,
                false,
                true,
            ...
            self::$pdf->writeHTMLCell(
                ...
                10,
                $galleryHeight + 9,
                view('cms.report.partials.pdf.company-about-page', compact('description'))->render(),
                0,
                0,
                0,
                1,
            ...
            $this->setUserFooter($this->user, $this->reportDraft->footer);
        }
    ...
    public function addSectionPage(
        ...
        self::$pdf->writeHTMLCell(
            ...
            $this->pageStartX + 5,
            $galleryHeight - 15,
            view('cms.report.partials.pdf.page-header-2', compact('title', 'pageAbout'))->render(),
            0,
            0,
            false,
            true,
        ...
        self::$pdf->writeHTMLCell(
            ...
            10,
            $galleryHeight + 10,
            view('cms.report.partials.pdf.page-about', compact( 'description'))->render(),
            0,
            0,
            0,
            1,
    ...
    public function addProjectPage(
        ...
        foreach ($descriptions as $index => $description) {
            ...
            $galleryHeight = $this->addProjectImages($imagesCount, $tmpImages, 3);
            ...
            self::$pdf->writeHTMLCell(
                ...
                $this->pageStartX + 5,
                $galleryHeight - 25,
                view('cms.report.partials.pdf.project-header-2', compact('name', 'address'))->render(),
                0,
                0,
                false,
                true,
    ...
    {
        $ratio = $this->coverImageWidthPx / self::$pdf->pixelsToUnits($this->coverImageWidthPx);
        $height = ($ratio * self::$pdf->getPageHeight()) * (2 / 3);
        $width = ($ratio * self::$pdf->getPageWidth());
        self::$pdf->Image(
            image_src($image, (int) $width * 2, (int) $height * 2),
            0,
            self::$pdf->getPageHeight() * (1 / 6),
            self::$pdf->getPageWidth(),
            self::$pdf->pixelsToUnits($height)
        );
        ...
        if ($coverBackgroundColor) {
            ...
            self::$pdf->Rect(
                self::$pdf->getPageWidth() * (1 / 5),
                0,
                self::$pdf->getPageWidth() - (self::$pdf->getPageWidth() * (2 / 5)),
            ...
            self::$pdf->SetAlpha($backgroundRgba['a']);
            self::$pdf->Rect(
                self::$pdf->getPageWidth() * (1 / 10),
                self::$pdf->getPageHeight() * (1 / 12),
                self::$pdf->getPageWidth() - (self::$pdf->getPageWidth() * (2 / 10)),
                self::$pdf->getPageHeight() - (self::$pdf->getPageHeight() * (2 / 12)),
                'S',
                [
                    'all' => [
                        'width' => 4,
                        ...
                        'join' => 'miter',
                        'dash' => 0,
                        'color' => [$backgroundRgba['r'], $backgroundRgba['g'], $backgroundRgba['b']]
                    ]
                ]
            );
            self::$pdf->SetAlpha(1);
            ...
            self::$pdf->Line(
                self::$pdf->getPageWidth() * (2 / 10),
                self::$pdf->getPageHeight() - (self::$pdf->getPageHeight() * (1 / 12)),
                self::$pdf->getPageWidth() - (self::$pdf->getPageWidth() * (2 / 10)),
                self::$pdf->getPageHeight() - (self::$pdf->getPageHeight() * (1 / 12)),
                [
                    'width' => 4,
    ...
    private function _addCompanyDetails(
        ...
        self::$pdf->writeHTMLCell(
            ...
            0,
            self::$pdf->getPageWidth() * (1 / 5),
            self::$pdf->getPageHeight() * (1 / 2.5),
            view(
                'cms.report.partials.pdf.company-logo-2',
